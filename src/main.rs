...
async fn main() {
    ...
    let app = Router::new()
        ...
        .route(
            "/motion_zones",
            get(get_motion_zones).post(save_motion_zones),
        )
        // 控制
        ...
        .route(
            "/timelapse/config",
            get(get_timelapse_cfg).post(save_timelapse_cfg),
        )
        // 事件日志
        ...
        .route(
            "/notify_config",
            get(get_notify_config).post(save_notify_config),
        )
        ...
        .route(
            "/onedrive_config",
            get(get_onedrive_config).post(save_onedrive_config),
        )
        .route("/onedrive_share", get(create_onedrive_share))
        .route(
            "/gdrive_config",
            get(get_gdrive_config).post(save_gdrive_config),
        )
        ...
        .route(
            "/privacy_masks",
            get(get_privacy_masks).post(save_privacy_masks),
        )
        // 运动热力图
        ...
        .route(
            "/rtsp_cameras",
            get(get_rtsp_cameras).post(save_rtsp_cameras),
        )
        // 安全配置
        .route(
            "/security_config",
            get(get_security_config).post(save_security_config),
        )
        // 录像限制
        .route(
            "/record_limits",
            get(get_record_limits).post(save_record_limits),
        )
        // 系统
