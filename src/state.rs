...
use crate::upload::{FtpConfig, GoogleDriveConfig, OneDriveConfig};
...
impl AppState {
    ...
    pub fn alert_allowed(&self) -> bool {
        ...
        if self.notify_suppressed.load(Ordering::Relaxed) {
            return false;
        }
        ...
        if rule.weekdays != 0xFF && (rule.weekdays & wd_bit) == 0 {
            return true;
        }
