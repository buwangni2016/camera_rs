...
pub async fn upload_all(
    ...
    let ct = if filename.ends_with(".avi") {
        "video/avi"
...
pub async fn upload_onedrive(
    ...
    if cfg.maton_api_key.is_empty() {
        return None;
    }
    let folder = if cfg.folder.is_empty() {
        "camera_rs"
    } else {
        &cfg.folder
    };
    ...
    let resp = client
        .put(format!(
            "{}/one-drive/v1.0/me/drive/root:/{}:/content",
            MATON_BASE, path
        ))
    ...
    if !cfg.create_share_links {
        return None;
    }
    let item_id = resp["id"].as_str()?;
    let share = client
        .post(format!(
            "{}/one-drive/v1.0/me/drive/items/{}/createLink",
            MATON_BASE, item_id
        ))
        .header("Authorization", format!("Bearer {}", cfg.maton_api_key))
...
pub async fn create_folder_share(cfg: &OneDriveConfig) -> Option<String> {
    ...
    let _ = client
        .put(format!(
            "{}/one-drive/v1.0/me/drive/root:/{}/.keep:/content",
            MATON_BASE, folder
        ))
        .header("Authorization", format!("Bearer {}", cfg.maton_api_key))
...
pub async fn upload_google_drive(cfg: &GoogleDriveConfig, filename: &str, data: &[u8], ct: &str) {
    ...
    let folder_id = if !cfg.folder_id.is_empty() {
        ...
        get_or_create_gdrive_folder(&client, &cfg.maton_api_key, &cfg.folder_name)
            .await
...
async fn get_or_create_gdrive_folder(
    ...
    let resp = client
        .get(format!(
            "{}/google-drive/drive/v3/files?q={}&fields=files(id)",
            MATON_BASE,
    ...
    // 创建文件夹
    let create = client
        .post(format!("{}/google-drive/drive/v3/files", MATON_BASE))
        .header("Authorization", format!("Bearer {}", api_key))
