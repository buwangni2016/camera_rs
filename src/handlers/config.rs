...
pub async fn save_config(
    ...
    if let Some(v) = p.smtp_host {
        cfg.smtp_host = v;
    }
    if let Some(v) = p.smtp_port {
        cfg.smtp_port = v;
    }
    if let Some(v) = p.email_from {
        cfg.from = v;
    }
    if let Some(v) = p.email_password {
        if !v.is_empty() {
            cfg.password = v;
        }
    }
    ...
    if let Some(v) = p.on_motion {
        cfg.on_motion = v;
    }
    if let Some(v) = p.on_unknown {
        cfg.on_unknown = v;
    }
    Json(OkResp {
        ok: true,
...
pub async fn test_notify(State(state): State<AppState>, jar: PrivateCookieJar) -> Json<OkResp> {
    if !is_authed(&jar) && !crate::PASSWORD.is_empty() {
        return Json(OkResp {
            ok: false,
            error: Some("Unauthorized".into()),
    ...
    tokio::spawn(async move {
        crate::notify::send_all(
            &cfg,
            crate::notify::NotifyEvent::Custom {
                title: "🔔 通知测试",
...
pub async fn get_alert_rule(
    ...
    jar: PrivateCookieJar,
) -> Json<crate::state::AlertTimeRule> {
    if !is_authed(&jar) && !crate::PASSWORD.is_empty() {
...
pub async fn upload_now(State(state): State<AppState>, jar: PrivateCookieJar) -> Json<OkResp> {
    if !is_authed(&jar) && !crate::PASSWORD.is_empty() {
        return Json(OkResp {
            ok: false,
            error: Some("Unauthorized".into()),
...
pub async fn get_gdrive_config(
    State(s): State<AppState>,
    jar: PrivateCookieJar,
) -> Json<crate::upload::GoogleDriveConfig> {
    if !is_authed(&jar) && !crate::PASSWORD.is_empty() {
        return Json(Default::default());
    }
...
pub async fn save_gdrive_config(
    State(s): State<AppState>,
    jar: PrivateCookieJar,
    Json(c): Json<crate::upload::GoogleDriveConfig>,
) -> Json<OkResp> {
    if !is_authed(&jar) && !crate::PASSWORD.is_empty() {
        return Json(OkResp::default());
    }
...
pub async fn get_ftp_config(
    State(s): State<AppState>,
    jar: PrivateCookieJar,
) -> Json<crate::upload::FtpConfig> {
    if !is_authed(&jar) && !crate::PASSWORD.is_empty() {
        return Json(Default::default());
    }
...
pub async fn save_ftp_config(
    State(s): State<AppState>,
    jar: PrivateCookieJar,
    Json(c): Json<crate::upload::FtpConfig>,
) -> Json<OkResp> {
    if !is_authed(&jar) && !crate::PASSWORD.is_empty() {
        return Json(OkResp::default());
    }
...
pub async fn save_schedule(
    State(s): State<AppState>,
    jar: PrivateCookieJar,
    Json(rules): Json<Vec<crate::schedule::ScheduleRule>>,
) -> Json<OkResp> {
    if !is_authed(&jar) && !crate::PASSWORD.is_empty() {
        return Json(OkResp::default());
    }
...
pub async fn save_users(
    ...
    for form in forms {
        if let Some(u) = users.iter_mut().find(|u| u.username == form.username) {
            ...
            u.role = if form.role == "admin" {
                crate::state::UserRole::Admin
            } else {
                crate::state::UserRole::Viewer
            };
            if let Some(pw) = &form.password {
                if !pw.is_empty() {
                    u.password_hash = crate::auth::hash_password(pw);
                }
            }
        } else {
            let pw_hash = form
...
pub async fn save_api_config(
    ...
    if c.api_key.is_empty() {
        c.api_key = crate::auth::generate_api_key();
    }
    *s.api_cfg.lock() = c;
    Json(OkResp {
        ok: true,
...
pub async fn get_timelapse_cfg(
    State(s): State<AppState>,
    jar: PrivateCookieJar,
) -> Json<crate::state::TimelapseConfig> {
    if !is_authed(&jar) && !crate::PASSWORD.is_empty() {
        return Json(Default::default());
    }
...
pub async fn save_timelapse_cfg(
    State(s): State<AppState>,
    jar: PrivateCookieJar,
    Json(c): Json<crate::state::TimelapseConfig>,
) -> Json<OkResp> {
    if !is_authed(&jar) && !crate::PASSWORD.is_empty() {
        return Json(OkResp::default());
    }
...
pub async fn get_watermark(
    State(s): State<AppState>,
    jar: PrivateCookieJar,
) -> Json<crate::state::WatermarkConfig> {
    if !is_authed(&jar) && !crate::PASSWORD.is_empty() {
        return Json(Default::default());
    }
...
pub async fn save_watermark(
    State(s): State<AppState>,
    jar: PrivateCookieJar,
    Json(c): Json<crate::state::WatermarkConfig>,
) -> Json<OkResp> {
    if !is_authed(&jar) && !crate::PASSWORD.is_empty() {
        return Json(OkResp::default());
    }
...
pub async fn get_privacy_masks(
    State(s): State<AppState>,
    jar: PrivateCookieJar,
) -> Json<Vec<crate::state::PrivacyMask>> {
    if !is_authed(&jar) && !crate::PASSWORD.is_empty() {
        return Json(vec![]);
    }
...
pub async fn save_privacy_masks(
    State(s): State<AppState>,
    jar: PrivateCookieJar,
    Json(masks): Json<Vec<crate::state::PrivacyMask>>,
) -> Json<OkResp> {
    if !is_authed(&jar) && !crate::PASSWORD.is_empty() {
        return Json(OkResp::default());
    }
...
pub async fn get_rtsp_cameras(
    State(s): State<AppState>,
    jar: PrivateCookieJar,
) -> Json<Vec<crate::state::RtspCamera>> {
    if !is_authed(&jar) && !crate::PASSWORD.is_empty() {
        return Json(vec![]);
    }
...
pub async fn save_rtsp_cameras(
    State(s): State<AppState>,
    jar: PrivateCookieJar,
    Json(cams): Json<Vec<crate::state::RtspCamera>>,
) -> Json<OkResp> {
    if !is_authed(&jar) && !crate::PASSWORD.is_empty() {
        return Json(OkResp::default());
    }
...
pub async fn get_security_config(
    State(s): State<AppState>,
    jar: PrivateCookieJar,
) -> Json<super::SecurityConfigSafe> {
    if !is_authed(&jar) && !crate::PASSWORD.is_empty() {
        return Json(super::SecurityConfigSafe::from(
            &crate::state::SecurityConfig::default(),
        ));
    }
...
pub async fn save_security_config(
    State(s): State<AppState>,
    jar: PrivateCookieJar,
    Json(c): Json<crate::state::SecurityConfig>,
) -> Json<OkResp> {
    if !is_authed(&jar) && !crate::PASSWORD.is_empty() {
        return Json(OkResp::default());
    }
    ...
    let password = if new_pw.is_empty() {
        sec.password.clone()
    } else {
        new_pw
    };
...
pub async fn get_record_limits(
    State(s): State<AppState>,
    jar: PrivateCookieJar,
) -> Json<crate::state::RecordLimits> {
    if !is_authed(&jar) && !crate::PASSWORD.is_empty() {
        return Json(Default::default());
    }
...
pub async fn save_record_limits(
    State(s): State<AppState>,
    jar: PrivateCookieJar,
    Json(c): Json<crate::state::RecordLimits>,
) -> Json<OkResp> {
    if !is_authed(&jar) && !crate::PASSWORD.is_empty() {
        return Json(OkResp::default());
    }
    ...
    (
        [
            ...
            (
                ...
                "attachment; filename=camera_rs_config.json",
            ),
        ],
        json,
    )
...
pub async fn import_config(
    ...
    if let Some(v) = data.get("notify") {
        if let Ok(c) = serde_json::from_value(v.clone()) {
            *state.notify_cfg.lock() = c;
        }
    }
    if let Some(v) = data.get("onedrive") {
        if let Ok(c) = serde_json::from_value(v.clone()) {
            *state.onedrive_cfg.lock() = c;
        }
    }
    if let Some(v) = data.get("gdrive") {
        if let Ok(c) = serde_json::from_value(v.clone()) {
            *state.gdrive_cfg.lock() = c;
        }
    }
    if let Some(v) = data.get("ftp") {
        if let Ok(c) = serde_json::from_value(v.clone()) {
            *state.ftp_cfg.lock() = c;
        }
    }
    if let Some(v) = data.get("schedule") {
        if let Ok(r) = serde_json::from_value(v.clone()) {
            *state.schedule_rules.lock() = r;
        }
    }
    if let Some(v) = data.get("alert_rule") {
        if let Ok(r) = serde_json::from_value(v.clone()) {
            *state.alert_time_rule.lock() = r;
        }
    }
    if let Some(v) = data.get("motion_zones") {
        if let Ok(z) = serde_json::from_value(v.clone()) {
            *state.motion_zones.write() = z;
        }
    }
    if let Some(v) = data.get("watermark") {
        if let Ok(c) = serde_json::from_value(v.clone()) {
            *state.watermark_cfg.lock() = c;
        }
    }
    if let Some(v) = data.get("privacy_masks") {
        if let Ok(m) = serde_json::from_value(v.clone()) {
            *state.privacy_masks.lock() = m;
        }
    }
    if let Some(v) = data.get("rtsp_cameras") {
        if let Ok(c) = serde_json::from_value(v.clone()) {
            *state.rtsp_cameras.lock() = c;
        }
    }
    if let Some(v) = data.get("security") {
        if let Ok(mut c) = serde_json::from_value::<crate::state::SecurityConfig>(v.clone()) {
            ...
            if c.password.trim().is_empty() {
                c.password = state.security.lock().password.clone();
            }
            *state.security.lock() = c;
        }
    }
    if let Some(v) = data.get("record_limits") {
        if let Ok(c) = serde_json::from_value(v.clone()) {
            *state.record_limits.lock() = c;
        }
    }
    Json(OkResp {
        ok: true,
