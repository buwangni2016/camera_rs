...
pub fn build_daily_report(state: &AppState) -> String {
    ...
    let today_motion: usize = events
        .iter()
        .filter(|e| matches!(e.kind, EventKind::Motion) && e.timestamp.starts_with(&today))
        .count();
    let today_photos: usize = events
        .iter()
        .filter(|e| matches!(e.kind, EventKind::Photo) && e.timestamp.starts_with(&today))
        .count();
    let today_records: usize = events
        .iter()
        .filter(|e| matches!(e.kind, EventKind::RecordStop) && e.timestamp.starts_with(&today))
        .count();
...
pub async fn send_daily_report(state: &AppState) {
    ...
    // 邮件
    {
        ...
        if cfg.enabled && !cfg.to.is_empty() {
            let r = report.clone();
            let _ = tokio::task::spawn_blocking(move || send_report_email(&cfg, &r)).await;
        }
    }
