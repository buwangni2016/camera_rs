...
pub enum NotifyEvent<'a> {
    Motion { count: u64, image: &'a [u8], image_url: Option<&'a str> },
    Recording { started: bool },
    CameraOnline { index: usize },
    CameraOffline { index: usize },
    Custom { title: &'a str, body: &'a str },
}
...
pub async fn send_all(cfg: &NotifyConfig, event: NotifyEvent<'_>) {
    ...
    let (title, body, image, image_url) = match &event {
        NotifyEvent::Motion { count, image, image_url } => (
            format!("⚠️ 运动侦测告警"),
            format!("检测到移动，累计触发 {} 次{}", count,
                image_url.map(|u| format!("\n📷 [查看截图]({})", u)).unwrap_or_default()),
            *image,
            *image_url,
        ),
        NotifyEvent::Recording { started } => (
            if *started { "🎥 开始录像".into() } else { "💾 录像已保存".into() },
            if *started { "摄像头开始录像".into() } else { "录像文件已保存".into() },
            &[] as &[u8],
            None,
        ),
    ...
    if cfg.telegram.enabled && !cfg.telegram.bot_token.is_empty() {
        let c = client.clone(); let cfg = cfg.telegram.clone();
    ...
    if cfg.dingtalk.enabled && !cfg.dingtalk.webhook_url.is_empty() {
        let c = client.clone(); let cfg = cfg.dingtalk.clone();
    ...
    if cfg.wecom.enabled && !cfg.wecom.webhook_url.is_empty() {
        let c = client.clone(); let cfg = cfg.wecom.clone();
        let t = title.clone(); let b = body.clone(); let iu = image_url.map(String::from);
        handles.push(tokio::spawn(async move {
    ...
    if cfg.serverchan.enabled && !cfg.serverchan.send_key.is_empty() {
        let c = client.clone(); let cfg = cfg.serverchan.clone();
    ...
    if cfg.bark.enabled && !cfg.bark.server_url.is_empty() {
        let c = client.clone(); let cfg = cfg.bark.clone();
        let t = title.clone(); let b = body.clone();
        handles.push(tokio::spawn(async move {
    ...
    if cfg.pushplus.enabled && !cfg.pushplus.token.is_empty() {
        let c = client.clone(); let cfg = cfg.pushplus.clone();
        let t = title.clone(); let b = body.clone(); let iu = image_url.map(String::from);
        handles.push(tokio::spawn(async move {
    ...
    if cfg.webhook.enabled && !cfg.webhook.url.is_empty() {
        let c = client.clone(); let cfg = cfg.webhook.clone();
        let t = title.clone(); let b = body.clone(); let iu = image_url.map(String::from);
        handles.push(tokio::spawn(async move {
...
async fn send_telegram(
    client: &reqwest::Client, cfg: &TelegramCfg,
    ...
    if cfg.send_photo && !image.is_empty() {
        let form = reqwest::multipart::Form::new()
            ...
            .part("photo", reqwest::multipart::Part::bytes(image.to_vec())
                .file_name("capture.jpg")
                .mime_str("image/jpeg")?);
        client.post(format!("https://api.telegram.org/bot{}/sendPhoto", cfg.bot_token))
            .multipart(form).send().await?;
    } else {
        client.post(format!("https://api.telegram.org/bot{}/sendMessage", cfg.bot_token))
...
async fn send_dingtalk(
    client: &reqwest::Client, cfg: &DingTalkCfg,
    ...
    let msg_type = if image_url.is_some() { "markdown" } else { "text" };
...
async fn send_wecom(
    client: &reqwest::Client, cfg: &WeComCfg,
    title: &str, body: &str, image_url: Option<&str>,
) -> anyhow::Result<()> {
...
async fn send_serverchan(
    client: &reqwest::Client, cfg: &ServerChanCfg,
    title: &str, body: &str,
) -> anyhow::Result<()> {
    client.post(format!("https://sctapi.ftqq.com/{}.send", cfg.send_key))
...
async fn send_bark(
    client: &reqwest::Client, cfg: &BarkCfg,
    title: &str, body: &str,
) -> anyhow::Result<()> {
    ...
    if !cfg.sound.is_empty() { payload["sound"] = cfg.sound.clone().into(); }
    if !cfg.group.is_empty() { payload["group"] = cfg.group.clone().into(); }
    if !cfg.icon.is_empty()  { payload["icon"]  = cfg.icon.clone().into(); }
...
async fn send_pushplus(
    client: &reqwest::Client, cfg: &PushPlusCfg,
    title: &str, body: &str, image_url: Option<&str>,
) -> anyhow::Result<()> {
    let content = if let Some(url) = image_url {
    ...
    if !cfg.topic.is_empty() { payload["topic"] = cfg.topic.clone().into(); }
...
async fn send_webhook(
    client: &reqwest::Client, cfg: &WebhookCfg,
    title: &str, body: &str, image_url: Option<&str>,
) -> anyhow::Result<()> {
    use std::time::{SystemTime, UNIX_EPOCH};
    ...
    let payload = if cfg.body_template.is_empty() {
        ...
        let rendered = cfg.body_template
            .replace("{event}", title)
            .replace("{message}", body)
            .replace("{image_url}", image_url.unwrap_or(""))
            .replace("{timestamp}", &ts);
        serde_json::from_str(&rendered)
            .unwrap_or_else(|_| serde_json::json!({"raw": rendered}))
    };
